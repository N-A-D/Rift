...
Threading has been added as a feature to the library but it requires C++17 for the standard's parallel algorithms (std::for_each, std::count_if, std::sort). System transformations are applied with as many threads as possible on the machine with the `rift::EntityManager::par_for_entities_with` function. This function differs from `rift::EntityManager::for_entities_with` in that system transformations cannot add/remove/destroy components as well as destroy entities. Moreover, updating a `rift::EntityManager` within a system transformation will lead to undefined behaviour. Note: this addition to the library is experimental and it *may* actually lead to degraded performance versus sequential transformations. In other words, use this new function only if it leads to increased throughput.
